    ...
    class SummaryManager {
        ...
        async optimizeSummary(target, userPrompt, rangeInput = "1") {
            ...
            if (userPrompt && userPrompt.trim()) {
                ...
                coreInstruction = `请对上述内容进行整合且精简优化，严格遵守以下核心协议：

1. 【格式纯净】：严禁使用 Markdown 符号（如 #、*、-、>），严禁加粗。仅使用纯文本和标点符号。
2. 【时空聚合】：同一时间段、同一地点内的多条碎片记录，必须合并为一段连贯、紧凑的描述。严禁拆分成流水账，严禁同一场景多次换行。
错误：
10:00-10:05 [餐厅] 张三紧盯着李四
10:05-10:30 [餐厅] 张三紧扣留了李四手机
正确：
0:00-10:30 [餐厅] 张三紧盯着李四并扣留了其手机，要求李四汇报行踪；李四拒绝配合并摔碎了杯子。
3. 【拒绝抽象】：严禁使用“宣示主权”、“暧昧气氛”、“心理博弈”等定义性词汇。必须保留具体的“行为动作”和“对话核心”来体现事情的前因后果及状态。
4. 【因果完整】：严禁为了精简而删除前因后果。必须保留导致状态变更、关系转折的关键逻辑链条。
5. 【客观口吻】：绝对客观记录。`;
            }
