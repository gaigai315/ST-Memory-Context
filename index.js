...
【记忆总结·强合并规则】
请严格执行"时空聚合"逻辑，禁止流水账式记录，严格剔除如"宣示主权、表明占有欲"等主观情感解读，仅保留客观行为（做/说）、具体计划、因果行动：

1. 📅 按日归档原则：
   - 同一日期的所有事件，必须合并在同一行记录。
   - 只有日期变更时，才允许换行/新增一行。

2. 📍 时空合并逻辑：
   - 若在同一地点发生了连续的多个互动（如对话、动作、冲突），必须合并为一个时间段。
   - 格式要求：\`开始时间-结束时间 [地点] 事件概要\`
   - 时间计算：取该场景第一件事的开始时间至最后一件事的结束时间。
...
4. ✅ 正确输出范例（请严格模仿此结构）：
   - 2024年03月15日·08:05-12:25 [地点A·教室/地点B·别墅] 角色A在公开场合向角色B赠送物品；角色C半路拦截并将角色B强行带至地点B，对其进行了肢体限制；角色D闯入现场打断了该行为；角色A最终抵达将角色B带离，并按计划为其佩戴了项圈道具；13:10-17:20 [地点C·公寓] 四名角色集结，向角色B展示了不利证据作为筹码，要求其签署《从属协议》；随后众人在客厅及书房对角色B执行了多轮仪式性行为。

5. 🚫 禁止事项：
   - 严禁将同一地点前后相差几分钟的对话拆分成两行。
   - 严禁单独罗列每一个动作（如"他进来了"、"他说话了"），请概括为"A进入房间与B发生争执"。
