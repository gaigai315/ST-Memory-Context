...
请按照输出格式输出总结内容，严禁包含任何角色扮演的剧情描写、开场白、结束语或非剧情相关的交互性对话（如"收到"、"好的"）：`;
    ...
    /**
     * 同步总结到世界书
     * 将总结内容保存到名为 Memory_Context_Auto 的独立世界书中
    ...
    async function syncToWorldInfo(content) {
        ...
        try {
            ...
            const contentWithTimestamp = `📅 最后更新：${timestamp}\n\n${content}`;

            // 构建世界书数据
            const payload = {
                name: "Memory_Context_Auto",
                data: {
                    entries: {
                        "0": {
                            uid: 0,
                            ...
                            keysecondary: [],
                            comment: `Auto-generated by Memory Context Plugin v${V}`,
